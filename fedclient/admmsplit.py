...
class Client():
    ...
    def one_step_forward(self,is_train=True):
        ...
        if self.conf.use_DP==True and is_train==True :
            self.local_embed = clip_activation(self.local_embed, max_norm=self.conf.max_per_sample_clip_norm)
            noise = torch.cuda.FloatTensor( self.local_embed.shape, device= self.local_embed.device ).normal_(mean=0, std=self.conf.DP_sigma * self.conf.max_per_sample_clip_norm)
            self.local_embed.add_(noise)
    ...
    def update_w(self):
        local_objective= self.local_loss(self.logits, 0, False)
    ...
    def get_hx(self, is_train=True):
        if True == is_train:
            ...
            return self.logits # local_embed = test_feature
