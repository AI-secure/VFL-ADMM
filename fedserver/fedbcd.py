...
class Coordinator(object):
    ...
    def init(self):
        ...
        self.linearhead= get_server_model(self.conf.server_model_type, num_classes=self.conf.num_classes, emb_dim=self.conf.emb_dim*self.num_clients, dropout_rate=self.conf.dropout_rate).to(self.conf.device)
    ...
    def run(self,  val_log=1 ):
        ...
        for epoch in range(self.conf.num_round+1):
            ...
            if epoch>0:  # train
                ...
                for batch_idx in range(self.train_num_steps): # iter over the batches 
                    commu_round+=1
                    ...
                    concat_emb = torch.cat([self.fed_clients[uid].embedding for uid, party in enumerate(self.party)], dim=1)
                    
                    outputs = self.linearhead(concat_emb)

                    labels= self.fed_clients[0].y
                    if self.randomized_label_privacy!= None:
                        targets =  self.fed_clients[0].targets
                        loss = self.labeldp_criterion(outputs, targets)
                    else:
                    ...
                    ''' local worker '''
                    for uid, party in enumerate(self.party):
                        additional_step= self.conf.local_admm_epoch -1
                        self.fed_clients[uid].one_step_backward(additional_step=additional_step)
    ...
    def save_models(self, epoch, name):
        ...
        _path = os.path.join(self.conf.model_path, "server{}.ckpt".format(name )) 
        torch.save({
            'epoch': epoch,
