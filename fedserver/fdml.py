...
from model.basic import LinearHead
...
class Coordinator(object):
    ...
    def init(self):
        ...
        for uid, party in enumerate(self.party):
            self.fed_clients[uid].model = create_whole_network(party.num_features , self.conf)
            if len(self.conf.gpu_ids)>1:
                print("torch.nn.DataParallel  gpu_ids",self.conf.gpu_ids) 
                ...
                print("one device",self.conf.device)
