...
class NoisedCIFAR(torch.utils.data.Dataset):
    def __init__(
        ...
        print("NoisedCIFAR, label_change", self.label_change)
...
class NoisedDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        originaldataset: torch.utils.data.Dataset,
        num_classes: int,
        randomized_label_privacy: RandomizedLabelPrivacy,
    ):
        self.originaldataset = originaldataset
        self.rlp = randomized_label_privacy
        targets = originaldataset.targets
        self.soft_targets = [self._noise(t, num_classes) for t in targets]
        # self.rlp.increase_budget()  # increase budget
        # calculate probability of label change
        num_label_changes = sum(
        ...
        self.label_change = num_label_changes / len(targets)
        print("NoisedDataset, label_change", self.label_change)
    ...
    def __len__(self):
        return self.originaldataset.__len__()

    def __getitem__(self, index):
        image, label = self.originaldataset.__getitem__(index)
        return image, self.soft_targets[index], label
